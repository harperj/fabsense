  ...
  try:
    ...
    if (arr == False):
      ...
      if crc:
        ...
        gyrx = float(split[15])/5000.0
        client.send(msg) # now we dont need to tell the client the address anymore
        f.write(str(gyrx) + ", ")      # str() converts to string  # str() converts to string

        msg = OSC.OSCMessage() #  we reuse the same variable msg used above overwriting it
        msg.setAddress("/gyro/y")
        msg.append(float(split[16])/5000.0)
        gyry = float(split[16])/5000.0
        client.send(msg) # now we dont need to tell the client the address anymore
        f.write(str(gyry) + ", ")      # str() converts to string

        msg = OSC.OSCMessage() #  we reuse the same variable msg used above overwriting it
        msg.setAddress("/gyro/z")
        msg.append(float(split[17])/5000.0)
        gyrz = float(split[17])/5000.0
        client.send(msg) # now we dont need to tell the client the address anymore
        f.write(str(gyrz) + ", ")      # str() converts to string
        ...
        msg = OSC.OSCMessage() #  we reuse the same variable msg used above overwriting it
        msg.setAddress("/net/gyro")
        gyr_net = gyrx*gyrx + gyry*gyry + gyrz*gyrz
        msg.append(gyr_net)
        client.send(msg) # now we dont need to tell the client the address anymore
        f.write(str(gyr_net) + ", ")      # str() converts to string

        msg = OSC.OSCMessage() #  we reuse the same variable msg used above overwriting it
        msg.setAddress("/net/mAvgGyro")
        gyr_mAvg = mAvg(gyr_net)
        msg.append(gyr_mAvg)
        client.send(msg) # now we dont need to tell the client the address anymore
        
        msg = OSC.OSCMessage() #  we reuse the same variable msg used above overwriting it
        msg.setAddress("/peak/gyro")
        gyr_slope = getSlope(gyr_mAvg)
        msg.append(gyr_slope)
        client.send(msg) # now we dont need to tell the client the address anymore
