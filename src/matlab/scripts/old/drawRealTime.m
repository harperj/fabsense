...
for i = 2:10
    ...
    %data(:,i) = norm(data(:,i),1);
end

% rms for acc
for i = 1:length(data)
 rms(i,1) = sqrt(sum(data(i,2:4).^2)/3);
end

% rms for gyr
for i = 1:length(data)
 rms(i,2) = sqrt(sum(data(i,5:7).^2)/3);
end
 
% rms for mag
for i = 1:length(data)
 rms(i,3) = sqrt(sum(data(i,8:10).^2)/3);
end
...
%    end 
    ...
    %plot RMS
    for j = 11:13
        plotData(:,j) = [plotData(2:end,j) ; rms(i,j-10)];
    end
